 ...
 * ---------------------------------------------------------------------------------------------------------------------
 * <p>— это поведенческий паттерн проектирования, который определяет скелет алгоритма, перекладывая ответственность
 * за некоторые его шаги на подклассы. Паттерн позволяет подклассам переопределять шаги алгоритма, не меняя его общей структуры.
 * ---------------------------------------------------------------------------------------------------------------------
 ...
 * <p>5. Создайте конкретные классы, унаследовав их от абстрактного класса. Реализуйте в них все недостающие шаги и хуки.</p>
 * ---------------------------------------------------------------------------------------------------------------------
 ...
 * ---------------------------------------------------------------------------------------------------------------------
 * <ul>
 * <li><strong>Фабричный метод</strong> можно рассматривать как частный случай <strong>Шаблонного метода</strong>.
 * Кроме того, <i>Фабричный метод</i> нередко бывает частью большого класса с <i>Шаблонными методами</i>.</li>
 *
 * <li><strong>Шаблонный метод</strong> использует наследование, чтобы расширять части алгоритма. </strong>
 * <strong>Стратегия</strong> использует делегирование, чтобы изменять выполняемые алгоритмы на лету.
 * <i>Шаблонный метод</i> работает на уровне классов. <i>Стратегия</i> позволяет менять логику отдельных объектов.</li>
 * </ul>
 * ---------------------------------------------------------------------------------------------------------------------
 * <p><strong>Примеры Шаблонных методов в стандартных библиотеках Java:</strong></p>
 * ---------------------------------------------------------------------------------------------------------------------
 * <p>Все не-абстрактные методы классов <strong>java.io.InputStream</strong>, <strong>java.io.OutputStream</strong>,
 * <strong>java.io.Reader</strong> и <strong>java.io.Writer</strong>.
 * <p>Все не-абстрактные методы классов <strong>java.util.AbstractList</strong>, <strong>java.util.AbstractSet</strong>
 * и <strong>java.util.AbstractMap</strong>.
 * <p><strong>javax.servlet.http.HttpServlet</strong>, все методы doXXX() по умолчанию возвращают HTTP-код 405
 * "Method Not Allowed". Однако вы можете переопределить их при желании.
 */
package architecture.patterns.basic.refactoring.guru.behavioral.template;